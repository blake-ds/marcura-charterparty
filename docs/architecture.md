...
|---|---|---|
| `shellvoy-4` *Safe berth* | ends `…in the giving of the order,` | Rest of the sentence is struck out on PDF page 7 — see [`eval/ground_truth.md`](../eval/ground_truth.md) row 4. |
| `additional-1` *Indemnity Clause* | ends `…in accordance with the English Law and each` | Heavy strike-through on PDF page 18–19 interrupts the legalese mid-sentence. Ground-truth row 1 documents this. |
| `additional-11` *Adherence to Voyage Instruction Clause* | ends `…such voyage orders are given timely` | PDF page 28 has a struck tail after this token. Confirmed against the source. |
