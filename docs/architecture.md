...
Char-level marking is the primary filter, but a strike rectangle that ends mid-word leaves orphan glyphs visible (an `indemnity` line covered up to x=180 leaves "demnity" rendered, leaking into the JSON). Both parsers run the same `pdf.line_vote_filter` over body candidates. The inline parser adds one extra pre-check — `_is_wholly_replaced` — that drops clauses whose anchor row is title-struck *and* whose body region is dominated by heavy-struck lines (the Canada / Sidi Kerir / Rotterdam pattern). Anchor detection scans every word, struck or visible, so wholly-struck clauses still delimit their neighbours even when nothing on the page survives.
...
The verifier currently flags a handful of clauses with `truncated_text_end_mid_sentence`. All are **known false positives** — the deterministic parser is faithfully reproducing the PDF, which itself ends mid-sentence at these points because the following text is struck out. They are kept as warnings (not silenced) because they remain genuine "look here" signals for a human reviewer.

| Clause | Verifier flag | Why it's a false positive |
|---|---|---|
| `shellvoy-4` *Safe berth* | text ends mid-sentence at `…in the giving of the order,` | Rest of the sentence is struck out on PDF page 7 — see [`eval/ground_truth.md`](../eval/ground_truth.md) row 4. |
| `additional-1` *Indemnity Clause* | text ends mid-sentence at `…in accordance with the English Law and each` | Heavy strike-through on PDF page 18–19 interrupts the legalese mid-sentence. Ground-truth row 1 documents this. |
| `additional-1` *Indemnity Clause* | "two distinct subjects, suggesting merged clauses" | The clause genuinely contains an indemnity-letter section followed by a legal-proceedings section, separated by a struck region. They form one clause in the source PDF; the LLM observation is real but the "merged" label is wrong. |
| `additional-11` *Adherence to Voyage Instruction Clause* | text ends mid-sentence at `…such voyage orders are given timely` | PDF page 28 has a struck tail after this token. Confirmed against the source. |

The exact set varies run-to-run — the LLM verifier is non-deterministic by design. If a *new* clause is flagged that is not listed here, treat that as a real signal worth a manual page-by-page check against [`eval/ground_truth.md`](../eval/ground_truth.md).
...
The brief is `{id, title, text}` and we deliver that. PDF visual formatting — **bold** emphasis (e.g. the *"For the avoidance of doubt…"* paragraph inside `shellvoy-1`), `(a)` / `(b)` sub-list indentation, in-clause paragraph breaks beyond what a vertical-gap heuristic catches — is not preserved; the textual content is complete and faithful, but the visual hierarchy is flattened. PyMuPDF exposes per-glyph font flags, so bold-aware paragraph break insertion is a clean future-work win.
